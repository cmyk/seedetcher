...
func generate(fname string, conv *Face) error {
	...
	datafile := fmt.Sprintf("%s.bin", outname) // Removes extra ppem value
	first, n := utf8.DecodeRuneInString(filepath.Base(outname))
	var name string
	name = string(unicode.ToTitle(first)) + strings.ReplaceAll(outname[n:], "/", "_")
	name = "Bold" + strings.TrimPrefix(filepath.Base(outname), "bold")
	...
	fmt.Fprintf(&output, "var %s = bitmap.NewFace(unsafe.Slice(unsafe.StringData(data%s), len(data%s)))\n",
		name, name, name)
	fmt.Fprintf(&output, "var data%s string\n", name)
	fmt.Println("Generated source code:\n", output.String())
	...
	gofile := fmt.Sprintf("%s.go", outname)
	if err := os.WriteFile(gofile, formatted, 0o600); err != nil {
		return err
	}
	fmt.Println("Writing binary file:", datafile, "Size:", len(data))
	err = os.WriteFile(datafile, data, 0o600)
	if err != nil {
