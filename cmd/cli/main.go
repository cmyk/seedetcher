...
func dumpPCL(sideCmd engrave.Plan, size backup.PlateSize, keyIdx int, output string) error {
	m, _ := bip39.ParseMnemonic(*mnemonic)
	qrData := seedqr.QR(m)
	f, err := os.Create(filepath.Join(output, fmt.Sprintf("plate-%d-side-%s.pcl", keyIdx, *side)))
	if err != nil {
		return err
	}
	defer f.Close()
...
func run() error {
	...
	if *serialDev != "" {
		return hammer(sideCmd, *serialDev)
	}
